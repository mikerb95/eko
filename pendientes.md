...
- [ ] **6 vulnerabilidades de dependencias sin resolver** (1 moderate, 5 high) — tres (`brace-expansion`, `fast-uri`, `tar`) se arreglan con `npm audit fix`, sin nada breaking: es correrlo. La cuarta, `path-to-regexp` vía `@vercel/routing-utils`, no tiene arreglo hacia adelante: el proyecto ya está en `@astrojs/vercel@11` y `--force` hoy propone **bajar** a la 8.0.4. Hay que esperar la corrección aguas arriba. Ver `AUDITORIA.md` hallazgo 3.
...
- [ ] **`AUDITORIA.md` hallazgo #7 hay que reescribirlo, no cerrarlo** — dice "contraseña en texto plano". El almacenamiento de usuarios ya no lo está: `src/lib/users.ts` usa PBKDF2-SHA256 con salt por usuario (`hashPassword` / `verifyPassword`). Pero desde el parche de acceso sí hay una credencial en texto plano en ese mismo archivo (`src/lib/users.ts:151-152`), por otro motivo. El hallazgo debe apuntar a la credencial hardcodeada, no al hash.
