...
function cleanSections(raw: any): Section[] {
  ...
  for (const s of raw) {
    ...
    if (s.type === 'list') {
      const items = Array.isArray(s.items) ? s.items.map((x: any) => String(x)).filter(Boolean) : []
...
export const POST: APIRoute = async ({ request }) => {
  ...
  const post: Post = {
    ...
    date: String(b.date ?? '').trim() || new Date().toLocaleDateString('es-CO', { day: 'numeric', month: 'short', year: 'numeric' }),
