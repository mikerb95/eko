...
| 2026-07-06 | **Fase 0 / M7 (usuarios, roles y autorización) implementada**: `src/lib/users.ts` (tabla `users`, PBKDF2-SHA256 vía WebCrypto, sin dependencias), login contra DB con bootstrap del primer admin desde `ADMIN_USERNAME`/`ADMIN_PASSWORD` si la tabla está vacía, sesión firmada con nombre + rol, autorización por rol en el middleware (lectura para cualquier sesión; escrituras: recolecciones → admin/operaciones/logística, contenido → admin/consultor, usuarios → solo admin), pestaña "Usuarios" en el panel (crear/editar/desactivar/eliminar, reset de contraseña), salvaguardas (no auto-degradarse, no borrar el último admin activo), y la bitácora de órdenes registra el nombre real del empleado. Verificado end-to-end con los tres roles. |
| 2026-07-29 | **Notificación por email implementada** (cierra el pendiente de Fase 1): `src/lib/email.ts` envía el aviso al equipo vía API REST de Resend (sin SDK, `fetch` + timeout de 8s), conectado a `POST /api/recolecciones` y `POST /api/contacto` con plantilla HTML + texto plano, `reply_to` al solicitante y link al panel. Diseño *fail-soft*: si el correo falla o las variables no están, la orden/mensaje se guarda igual y el visitante recibe su confirmación (verificado con key inválida → 401 logueado, respuesta 200). **Pendiente de activación:** el recurso de Resend en el Marketplace no se pudo provisionar porque exige un dominio de envío y el dominio de Ekosolv sigue sin definirse (ver `src/lib/site.ts`); faltan `RESEND_API_KEY`, `NOTIFY_FROM` y `NOTIFY_EMAIL`. |
| 2026-08-03 | **§6 (Zoho) implementada en su primer tramo**: bandeja de salida `zoho_outbox` con reintentos y estados, cliente OAuth con CRM v8 y Books v3 (`src/lib/zoho.ts`), mapeo de contactos y órdenes a Leads (`src/lib/zohoMap.ts`), enganche en `POST /api/contacto` y `POST /api/recolecciones`, y endpoint de operación `GET/POST /api/admin/zoho` (sync, retry, discard) restringido a rol admin. Verificado end-to-end en local. **Pendiente de activación:** faltan las credenciales del self-client de Zoho. Detalle en `docs/plan-zoho.md`. |
| 2026-08-03 | **Bloqueador de producción detectado**: el proyecto en Vercel no tiene base de datos, así que el panel responde 500 y los formularios públicos pierden lo que reciben. Nada de lo anterior está realmente operativo para el cliente hasta que se provisione Turso. Ver `pendientes.md` y `AUDITORIA.md` hallazgo 9. |
...
Punto de partida (julio de 2026): la web recibía el formulario "Agenda una recolección" pero **no lo persistía ni lo enrutaba a nadie** (el form no tenía handler); la operación vivía en Zoho y el admin web solo gestionaba contenido. Ese diagnóstico es el que originó este plan; el estado real de cada módulo está en la tabla de avance de arriba.
...
|---|---|---|---|---|
| **0. Fundaciones** | 1–2 sem | M7 (usuarios/roles/auditoría), migraciones, layout del panel con navegación por módulos | Habilita todo lo demás; elimina la credencial única insegura | **Hecho (2026-07-06)** |
| **1. Recolecciones** | 2–3 sem | M1 completo + persistencia del form público + notificaciones email | Cero solicitudes perdidas; visibilidad diaria inmediata | **Hecho (2026-07-29)**; el email queda inerte hasta definir el dominio de envío |
